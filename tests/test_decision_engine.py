...
from app.decision_engine import TransactionFacts, evaluate
...
def test_normal_transaction_is_approved():
    verdict = evaluate(TransactionFacts(amount=100, recent_transaction_count=1, is_blocked=False), **LIMITS)
    assert verdict.decision == Decision.APPROVE


def test_blocked_card_is_declined():
    verdict = evaluate(TransactionFacts(amount=100, recent_transaction_count=1, is_blocked=True), **LIMITS)
    assert verdict.decision == Decision.DECLINE
    assert verdict.reason == "card_blocklisted"


def test_amount_over_threshold_is_declined():
    verdict = evaluate(TransactionFacts(amount=5001, recent_transaction_count=1, is_blocked=False), **LIMITS)
...
def test_amount_exactly_at_threshold_is_approved():
    ...
    verdict = evaluate(TransactionFacts(amount=5000, recent_transaction_count=1, is_blocked=False), **LIMITS)
    assert verdict.decision == Decision.APPROVE


def test_velocity_over_limit_is_declined():
    verdict = evaluate(TransactionFacts(amount=100, recent_transaction_count=6, is_blocked=False), **LIMITS)
    assert verdict.reason == "velocity_exceeded"


def test_velocity_exactly_at_limit_is_approved():
    verdict = evaluate(TransactionFacts(amount=100, recent_transaction_count=5, is_blocked=False), **LIMITS)
    assert verdict.decision == Decision.APPROVE


def test_blocked_card_beats_amount_rule():
    verdict = evaluate(TransactionFacts(amount=9999, recent_transaction_count=1, is_blocked=True), **LIMITS)
    assert verdict.reason == "card_blocklisted"